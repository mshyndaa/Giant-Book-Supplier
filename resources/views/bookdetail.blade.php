...
<body>
    ...
    <header>
      ...
      <div class="card p-5 m-5 " style="width: 50rem; height : auto; margin:auto; ">
        <img src="{{url($book->image)}}" class="card-img-top" style="object-fit:cover; height:100;" alt="...">
        <div class="card-body">
          <h3 class="fw-bold">{{$book->title}}</h3>
          <h4 class="fw-light">Author : {{$book->author}} </h4>
          <h4 class="fw-light">Publisher :  {{$publisher_name}}  </h4>
          <h4 class="fw-light" >Year :  {{$book->year}} </h4>
          <h4 class="fw-light">Sinopsis :  {{$book->synopsis}} </h4>
