     ...
     <div class=" navigation text-center ">
      ...
      <div class="card border-light text-bg-dark p-5 m-5 mt-5 " style="width: 45rem; height : 45rem; margin:auto; ">
          <div class="card-body-dark text-center">
            <img src="{{url($publisher->image)}}" style="object-fit:cover; width : 20rem; height : auto;" alt="">
            <h2 class="fw-bold m-3"> {{$publisher->name}}</h2>
            <h5 class="fw-light">{{$publisher->address}} </h5>
            <h5 class="fw-light">{{$publisher->phone}} </h5>
            <h5 class="fw-light" >{{$publisher->email}}</h5>
        </div>
      </div>
    </div>
      {{-- Publisher Book Detail --}}
      <div class="d-flex justify-content-center">
